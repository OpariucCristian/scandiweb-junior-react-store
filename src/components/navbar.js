        ...
        return (
            ...
            <nav id="nav">
                ...
                <div id="category-container">
                    ...
                    <h3 className="category-text">ALL</h3></label>
                    

                    <input className="category-radio" 
                    id="tech" name="categories" type="radio" onClick={()=> store.dispatch({type:'UPDATE_CATEGORY',val:"tech"})}/>
                    <label htmlFor="tech" className="category">
                    <h3 className="category-text">TECH</h3></label>

                    <input className="category-radio" 
                    id="clothes" name="categories" type="radio" onClick={()=> store.dispatch({type:'UPDATE_CATEGORY',val:"clothes"})}/>
                    <label htmlFor="clothes" className="category">
                        <h3 className="category-text">CLOTHES</h3></label>
                ...
                <div className="dim-page" id={!this.state.isMinicartOpen? "minicart-container-hide" : ""}> </div>
                <div id="buttons-container">
                    <div>
                      <button id="button-currency"
                       onClick={()=>this.setState({isCurrencyDropdownOpen: !this.state.isCurrencyDropdownOpen})}>
                           
                        <h3 >{this.currencyHandler(this.props.currency)}</h3>

                        {this.state.isCurrencyDropdownOpen?<svg width="8" height="4" viewBox="0 0 8 4" fill="none" 
                      ...
                      <div className="button-dropdown-currency" 
                      id={this.state.isCurrencyDropdownOpen ? "button-dropdown-currency-show" : ""}>
                      <button className="currency-button"  onClick={()=> store.dispatch({type:'UPDATE_CURRENCY',val:"USD"})}>
                      <h3 className="currency-button-text">$ USD</h3></button>
                      <button className="currency-button" onClick={()=> store.dispatch({type:'UPDATE_CURRENCY',val:"GBP"})}>
                      <h3 className="currency-button-text">£ GBP</h3></button>
                      <button className="currency-button" onClick={()=> store.dispatch({type:'UPDATE_CURRENCY',val:"AUD"})}>
                      <h3 className="currency-button-text">$ AUD</h3></button>
                      <button className="currency-button" onClick={()=> store.dispatch({type:'UPDATE_CURRENCY',val:"JPY"})}>
                      <h3 className="currency-button-text">¥ JPY</h3></button>
                      <button className="currency-button" onClick={()=> store.dispatch({type:'UPDATE_CURRENCY',val:"RUB"})}>
                      <h3 className="currency-button-text">₽ RUB</h3></button>
                      </div>
                    </div>
                    
                    <button id="button-cart" onClick={()=>this.setState({isMinicartOpen: !this.state.isMinicartOpen})}>
                    ...
                    {this.getTotalItems() >0 && <div className="cart-current-total">
                        <h4 className="cart-current-total-text">{this.getTotalItems()}</h4>
                    </div>}
                ...
                <MiniCart isOpen={this.state.isMinicartOpen} currency={this.props.currency}/>
            </nav>
        )
    }
}
